document.addEventListener('DOMContentLoaded', () => {
    ...
    function populateTokyoDistricts() {
        ...
        const tokyoDistrictOrder = [
            ...
            "羽村市", "あきる野市", "西東京市",
            "瑞穂町", "日の出町", "檜原村", "奥多摩町",
            "大島町", "八丈町", "新島村", "神津島村", "三宅村", "御蔵島村", "青ヶ島村", "小笠原村"
        ];

        // Sort districts based on predefined order
        const sortedDistricts = Array.from(districts).sort((a, b) => {
            const indexA = tokyoDistrictOrder.indexOf(a);
            const indexB = tokyoDistrictOrder.indexOf(b);

            if (indexA === -1 && indexB === -1) return a.localeCompare(b, 'ja'); // Fallback for unknown districts
            if (indexA === -1) return 1; // Unknown district comes last
            if (indexB === -1) return -1; // Unknown district comes last
